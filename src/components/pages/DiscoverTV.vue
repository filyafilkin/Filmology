<template>
  <v-app>
    <section class="discover container">
      <h2 class="discover__title">TVShows</h2>
      <div class="discover__sort">
        <form class="discover__form">
          ...
          <input
            ...
            @change="getContent"
          />
          <label for="on_the_air" class="discover__label">On the air</label>
        </form>
      </div>
      <div class="movies-list container">
        <div class="movie" v-for="tv in tvseries" :key="tv.id">
...
export default {
  ...
  methods: {
    getContent() {
      this.axios
        .get(
          `https://api.themoviedb.org/3/tv/${this.selectedValue}?api_key=${this.apikey}&page=${this.pagenum}`
        )
        .then((response) => {
